...
from .ui_deimwidget import Ui_Form
from deim.deim import DeimProcess
from systemwidget import SystemWidget, SystemRunThread


class DeimWidget(SystemWidget, Ui_Form):
    ...
    def __init__(self):
        ...
        # 程序运行
        self.system_process = DeimProcess()
    ...
    def saveConfig(self):
        ...
        config_path = './memory/deim/' + filename + '.ini'
    ...
    def loadConfig(self, filename):
        if filename == '新配置':
            ...
            return
        path = pathlib.Path('./memory/deim/' + filename + '.ini')
        # 创建实例对象
        config = configparser.ConfigParser()
        # 读取配置文件
        config.clear()
    ...
    def showEditWidget(self):
        ...
        if config_name != '新配置':
            self.editWidget.setTitle(title='设置星像分割' + config_name + '.ini')
            self.editWidget.load(system='deim', process=self.getProcessList(),
                                 new_config=False, config_path='./memory/deim/' + config_name + '.ini',
                                 aofc_object_path=object_path, aofc_object_wildcard=object_wildcard)
        else:
            self.editWidget.setTitle(title='新建星像分割')
            self.editWidget.load(system='deim', process=self.getProcessList(),
